...
/**
 * Base class for Args4J Tests. It instantiates the test object, the CmdLineParser for that test
 * object and provides a String array for passing to the parser.
 *
 * @author Jan Materne
 */
public abstract class Args4JTestBase<T> extends TestCase {
  ...
  /**
   * Returns the Locale to use for the tests. Defaults to use the US-Locale but should be
   * overwritten by classes which tests I18N.
   */
  ...
  public void assertUsageLength(int expectedLength) {
    assertEquals(
        "Wrong amount of lines in usage message", expectedLength, getUsageMessage().length);
  }
