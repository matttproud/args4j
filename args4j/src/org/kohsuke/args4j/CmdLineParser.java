...
public class CmdLineParser {
   ...
   *        will receive values. If this is null, the processing will be skipped, which is useful if
  ...
  /**
   * Programmatically defines an argument (instead of reading it from annotations like you normally
   * do.)
   *
  ...
  /**
   * Programmatically defines an option (instead of reading it from annotations like you normally
   * do.)
   *
   * @param setter the setter for the type
   * @param o the Option
  ...
  protected OptionHandler createOptionHandler(OptionDef o, Setter setter) {
    ...
    if (h == OptionHandler.class) {
      ...
      if (Enum.class.isAssignableFrom(t)) return new EnumOptionHandler(this, o, setter, t);

      handlerType = handlerClasses.get(t);
      if (handlerType == null) throw new IllegalAnnotationError(Messages.UNKNOWN_HANDLER.format(t));
    } else {
   ...
   *
   * <p>
   * This method produces a string like " -d &lt;dir> -v -b", which is useful for printing a command
   * line example, perhaps as a part of the usage screen.
   *
   *
   ...
   *        this resource bundle, and the associated value is printed. See {@link Option#metaVar()}.
   ...
   *         Otherwise, this method returns a string that contains a space at the beginning (but not
   *         at the end.) This allows you to do something like:
   *
   ...
   * Prints the list of options and their usages to the screen.
   *
   * @param rb if this is non-null, {@link Option#usage()} is treated as a key to obtain the actual
  ...
  private void printOption(PrintWriter out, OptionHandler handler, int len, ResourceBundle rb) {
    ...
    for (int i = 0; i < Math.max(namesAndMetas.size(), usages.size()); i++) {
      ...
      String format = (nameAndMeta.length() > 0) ? " %1$-" + widthMetadata + "s : %2$-1s"
  ...
  private int getPrefixLen(OptionHandler h, ResourceBundle rb) {
    if (h.option.usage().length() == 0) return 0;
  ...
  private class CmdLineImpl implements Parameters {
    ...
    public String getParameter(int idx) throws CmdLineException {
      if (pos + idx >= args.length) throw new CmdLineException(
          CmdLineParser.this, Messages.MISSING_OPERAND.format(getOptionName()));
      return args[pos + idx];
    }
   ...
   *
   * @throws CmdLineException if there's any error parsing arguments, or if {@link Option#required()
   *         required} option was not given.
   */
  public void parseArgument(final String... args) throws CmdLineException {
    ...
    for (OptionHandler handler : options)
      if (handler.option.required() && !present.contains(handler)) throw new CmdLineException(
          this, Messages.REQUIRED_OPTION_MISSING.format(handler.option.toString()));

    // make sure that all mandatory arguments are present
    for (OptionHandler handler : arguments)
      if (handler.option.required() && !present.contains(handler)) throw new CmdLineException(
          this, Messages.REQUIRED_ARGUMENT_MISSING.format(handler.option.toString()));
  }
  ...
  public static void registerHandler(Class valueType, Class<? extends OptionHandler> handlerClass) {
    if (valueType == null || handlerClass == null) throw new IllegalArgumentException();
